...
void Publish(char *radar_data, char *topic)
{
    if (MQTT_CONNEECTED)
    {
        // ESP_LOG_BUFFER_HEX("after PUBLISH", radar_data, sizeof(mmwHeader));
        // Convert the data to json
        printf("Size of radaframe is %d\n", sizeof(radarFrame_t));
        esp_mqtt_client_publish(client, topic, radar_data, sizeof(radarFrame_t), 1, 0);
